  ...
  return (
    <header className="filters">
      ...
      <div style={{ display: "flex", gap: "15px", justifyContent: "space-around" }}>
        <button onClick={changeSelection} id='libros-disponibles' className="change-selection active">Libros disponibles: {filteredBooks.length}</button>
        <button onClick={changeSelection} id='libros-guardados' className="change-selection">Libros guardados: {booksSaved.length}</button>
      </div>
    </header>
  )
}
